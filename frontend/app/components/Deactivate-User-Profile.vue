  ...
  >
    ...
    >
      ...
      <form @submit.prevent="submitData">
        ...
        <input type="text" name="isActive" v-model="formData.isActive">
...
const submitData = async () => {
    ...
    const response = await profile.DeactivateUser(userId.value, formData.value)

    if(response) navigateTo("/")
};

onMounted(async () => {
  sessionData.value = await sessionStore.getSession();
  userId.value = sessionData.value.userId;
  console.log("Session Loaded:", sessionData.value);
});
